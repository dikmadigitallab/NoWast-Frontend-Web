...
export default function ListagemSetores() {
    ...
    const { data: setores, pages } = useGet({ pageNumber: pagination.pageNumber, pageSize: pagination.pageSize, disablePagination: false, url: "sector", query: search.query });
                    ...
                    isFilter && (
                        <Box>
                            <Box className="flex gap-3 justify-between items-center">
                                <TextField value={search.query} onChange={(e) => setSearch({ ...search, query: e.target.value })} variant="outlined" label="Nome do setor" className="w-[100%]" sx={formTheme} />
                            </Box>
