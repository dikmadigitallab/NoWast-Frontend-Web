...
export default function ListagemProduto() {
    ...
    const { data: produtos, pages } = useGet({ pageNumber: pagination.pageNumber, pageSize: pagination.pageSize, url: 'product', query: search.query, responsibleManagerId: search.responsibleManagerId, buildingId: search.buildingId });
                    ...
                    isFilter && (
                        <Box>
                            <Box className="flex gap-3 justify-between items-center">
                                <TextField
                                    value={search.query}
                                    onChange={(e) => setSearch({ ...search, query: e.target.value })} variant="outlined" label="Nome do produto" className="w-[100%]" sx={formTheme} />
